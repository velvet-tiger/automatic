...
#[tauri::command]
pub async fn list_agents_with_projects() -> Result<String, String> {
    let handle = std::thread::Builder::new()
        .name("list_agents_thread".into())
    ...
    handle.join().unwrap_or_else(|_| Err("list_agents_with_projects thread panicked".to_string()))
}
