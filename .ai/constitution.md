...
- **Building on Windows under Parallels:** Two issues to be aware of:
  1. **Shared folder temp file errors:** Rust compilation fails on Parallels shared folders (the `U:\` or similar mapped drive) with `failed to remove temporary directory: The parameter is incorrect. (os error 87)`. This happens because the shared macOS filesystem does not support certain Windows temp file operations that Rust's archive builder requires. **Fix:** Set the `CARGO_TARGET_DIR` environment variable to point to a local Windows drive (e.g. `C:\cargo-target\automatic-win`) so build artifacts are written to NTFS instead of the shared filesystem. Set it per-session in PowerShell (`$env:CARGO_TARGET_DIR = "C:\cargo-target\automatic-win"`) or permanently via `[System.Environment]::SetEnvironmentVariable("CARGO_TARGET_DIR", "C:\cargo-target\automatic-win", "User")`. Do **not** add this to `.cargo/config.toml` as it would break macOS/Linux builds.
- **TLS backend: native-tls (not rustls):** Both `reqwest` and `tauri-plugin-updater` are configured with `native-tls` instead of the default `rustls`. This is intentional — `rustls` depends on the `ring` crate, which requires `clang` for ARM64 assembly compilation on Windows. Getting clang to work correctly under ARM64 Windows (Parallels on Apple Silicon) is unreliable. Using `native-tls` avoids this entirely by delegating to the platform's built-in TLS: Schannel on Windows, Security.framework on macOS. Do not add `rustls` features to these crates without understanding the ARM64 Windows build implications.
