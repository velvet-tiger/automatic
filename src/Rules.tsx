      ...
      <div className="w-64 flex-shrink-0 flex flex-col border-r border-[#33353A] bg-[#1A1A1E]/50">
        ...
        <div className="flex-1 overflow-y-auto py-2 custom-scrollbar">
          {rules.length === 0 && !isCreating ? (
            <div className="px-4 py-3 text-[13px] text-[#8A8C93] text-center">No rules yet.</div>
          ) : (
            <ul className="space-y-1 px-2">
              {isCreating && (
                <li className="flex items-center gap-3 px-3 py-2.5 rounded-lg bg-[#2D2E36]">
                  ...
                  <span className="text-[13px] text-[#E0E1E6] italic">New Rule...</span>
                </li>
              )}
              {rules.map(entry => {
                  ...
                  <li key={entry.id} className="group relative">
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-[10px] text-[#8A8C93] truncate">{entry.id}</div>
                      </div>
                    </button>
                    <button
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-[#222327]">
        ...
        {(selectedId || isCreating) ? (
                ...
                {isCreating ? (
                  <div className="flex flex-col gap-1.5 min-w-0">
                    <input
                      ...
                      autoFocus
                      className="bg-transparent border-none outline-none text-[14px] font-medium text-[#E0E1E6] placeholder-[#8A8C93]/50 w-72"
                    />
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        className="bg-transparent border-none outline-none text-[11px] text-[#8A8C93] placeholder-[#8A8C93]/40 font-mono w-72"
                      />
                    </div>
                  </div>
                ) : isEditing ? (
                  <div className="flex flex-col gap-0.5 min-w-0">
                    <input
                      ...
                      placeholder="Display Name"
                    />
                    <span className="text-[10px] text-[#8A8C93] font-mono">{selectedId}</span>
                  </div>
                ) : (
                  <div className="flex flex-col gap-0.5 min-w-0">
                    <h3 className="text-[14px] font-medium text-[#E0E1E6] truncate">{selectedEntry?.name || displayName}</h3>
                    <span className="text-[10px] text-[#8A8C93] font-mono">{selectedId}</span>
                  </div>
                )}
              </div>
            ...
            <div className="flex-1 relative">
              {isEditing ? (
                ...
                <div className="absolute inset-0 overflow-y-auto p-6 font-mono text-[13px] whitespace-pre-wrap text-[#E0E1E6] leading-relaxed custom-scrollbar">
                  {ruleContent || <span className="text-[#8A8C93] italic">This rule is empty. Click edit to add content.</span>}
                </div>
              )}
            </div>
          </div>
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-center p-8">
            ...
            <h2 className="text-lg font-medium text-[#E0E1E6] mb-2">No Rule Selected</h2>
