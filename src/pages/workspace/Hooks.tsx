...
import { LockCell } from "../../components/ProtectionBadge";
...
// Hooks provided by a plugin cannot be edited or deleted from here.
const isDeletable = (entry: HookEntry) => !entry.plugin_id;
...
export default function Hooks() {
  ...
  const recentIds = useRecentlyAdded("hooks", recentRefresh);
  ...
  async function handleBulkDelete() {
    const targets = hooks.filter(h => selection.selectedIds.has(h.id) && isDeletable(h));
    if (targets.length === 0) return;

    const preview = targets.slice(0, 10).map(t => `• ${t.name}`).join("\n");
    const overflow = targets.length > 10 ? `\n…and ${targets.length - 10} more.` : "";
    const message = `Delete ${targets.length} hook${targets.length === 1 ? "" : "s"}?\n\n${preview}${overflow}\n\nThis cannot be undone.`;
    const confirmed = await ask(message, { title: "Delete Hooks", kind: "warning" });
    if (!confirmed) return;
    ...
    if (failed.length > 0) {
      ...
      setError(`Failed to delete ${failed.length} hook${failed.length === 1 ? "" : "s"}:\n${detail}${more}`);
    } else {
      setError(null);
    }
  }
  ...
  const filteredHooks = hooks.filter(h =>
    ...
    || h.agent.toLowerCase().includes(searchLower)
    || h.event.toLowerCase().includes(searchLower)
  );

  const selection = useBulkSelection(filteredHooks, h => h.id, isDeletable);
  const drawerOpen = isCreating || !!selectedEntry;

  const renderTableRow = (entry: HookEntry) => {
      ...
      >
        <td className="px-3 py-2 w-9" onClick={(e) => e.stopPropagation()}>
          {deletable ? (
            ...
            <LockCell tooltip={`Provided by plugin ${entry.plugin_id} — cannot be deleted.`} />
          )}
        </td>
        ...
        <td className="px-3 py-2 min-w-0">
          <div className="flex items-center gap-2 min-w-0">
            <div className="min-w-0">
              ...
              <div className="text-[11px] text-text-muted truncate mt-0.5">{entry.agent} · {entry.event}</div>
            </div>
  ...
  return (
    <div className="flex h-full w-full flex-col bg-bg-base">
        ...
        {selection.totalSelected > 0 && (
          <div className="flex items-center justify-between px-4 py-2 border-t border-border-strong/30 bg-brand/5">
            <span className="text-[12px] text-text-base">
              {selection.totalSelected} hook{selection.totalSelected === 1 ? "" : "s"} selected
              {bulkProgress && (
      ...
      <AssetTable
        ...
        emptyState={
          <>
            <div className="w-14 h-14 mx-auto mb-5 rounded-2xl bg-icon-skill/12 border border-icon-skill/20 flex items-center justify-center">
              <Webhook size={22} className="text-icon-skill" strokeWidth={1.5} />
            </div>
            <h2 className="text-[15px] font-medium text-text-base mb-2">No hooks yet</h2>
        ...
        noMatchState={
          <p className="text-[13px] text-text-muted">
            {searchLower ? `No hooks match "${search}".` : "No hooks yet."}
          </p>
        }
        columns={[
        ...
        selection={{
          ...
          ariaLabel: "Select all visible deletable hooks",
        }}
        recentIds={recentIds}
      />
      ...
      <AssetDrawer open={drawerOpen} onClose={closeDrawer} isEditing={isEditing} closeButtonTopClassName="top-4">
        <div className="min-h-[44px] pl-5 pr-10 py-2 border-b border-border-strong/40 flex items-center justify-between bg-bg-base/30 shrink-0">
