...
- New agent tool: Kimi Code (Moonshot AI's coding CLI, www.kimi.com/code). Syncs project instructions to the shared `AGENTS.md`, skills to the cross-tool `.agents/skills/` hub Kimi reads natively, MCP servers to a dedicated `.kimi-code/mcp.json` under `mcpServers` (stdio, HTTP, and SSE transports), and sub-agents to `.kimi-code/agents/`. Commands and hooks are deliberately not synced this release: Kimi's slash-command surface overlaps with skills (`/skill:<name>`), and hooks are documented only at the user scope (`~/.kimi-code/config.toml`), which project sync must not touch. ([ffa8a2c](https://github.com/velvet-tiger/automatic/commit/ffa8a2c))
- New agent tool: Z Code (Z.ai's desktop agentic development environment, zcode.z.ai). Syncs project instructions to the shared `AGENTS.md`, skills to `.zcode/skills/`, and MCP servers merged into the shared `.zcode/config.json` under `mcpServers` — the user's other config keys are preserved on both sync and removal. Hooks, commands, and sub-agents are deliberately not synced: Z Code ignores project-level hooks for security, and its workspace-level command/sub-agent paths are undocumented (sub-agents are beta at workspace scope). ([a96fcc1](https://github.com/velvet-tiger/automatic/commit/a96fcc1))
- Library sections (Instructions, Commands, Sub-Agents, Rules, Hooks, Tools, Providers, MCP Servers, Templates) now share the same table + drawer interface introduced for Skills, built on new shared components (`AssetTable`, `AssetDrawer`, `ProtectionBadge`, `useBulkSelection`) that replace ~900 lines of copy-pasted per-section code. Tools and Providers use a read-only variant (no create/delete, no bulk select) since neither is a CRUD list. Templates keeps its full composer inside the new drawer shell. ([3679cdb](https://github.com/velvet-tiger/automatic/commit/3679cdb))

### Fixed

- Adding a project against a directory that already contained `.automatic/project.json` no longer dead-ends with a refusal banner. The Add Project wizard pre-checks the directory via a new `inspect_project_directory` command; when it finds an orphan config (no registry entry pointing at the directory), a new dialog offers Import (adopt the on-disk config via `import_existing_project`), Remove and start fresh (wipe it via `delete_project_config`), or Cancel. This unblocks fresh clones of previously-tracked repos, moved `~/.automatic`, and manual registry cleanups. Backend covered by eight new unit tests. ([0b7f764](https://github.com/velvet-tiger/automatic/commit/0b7f764))
- MCP servers imported from Cursor no longer copy the literal `${workspaceFolder}` / `${userHome}` placeholders into every other agent's config. Cursor emulates VS Code and expands these natively, so they were reaching Automatic's registry as literal strings and silently breaking downstream servers under Claude Code, Codex, Gemini, and others. Substitution happens in `prepare_mcp_servers` — the choke point both sync and drift already route through — so the file on disk and the drift baseline stay in lockstep. `${env:FOO}` is left alone, since it belongs to Automatic's inherited-env system. ([f73590b](https://github.com/velvet-tiger/automatic/commit/f73590b))
- The Providers page no longer auto-opens the drawer on entry. The prior `agents[0]` fallback plus `localStorage` rehydration meant a stale selection reopened the drawer over the list every time the page loaded. `selectedId` now starts at `null`, is not persisted, and a stale value pointing at a removed agent is cleared instead of falling back to the first row. ([24bbe8b](https://github.com/velvet-tiger/automatic/commit/24bbe8b))
- FirstRunWizard's agent list now includes Pi and Zed (previously missing), and `vite.config.ts` now ignores `.pi/` so syncing a project with the Pi agent while running `make dev` no longer triggers HMR reloads. ([e91a8d9](https://github.com/velvet-tiger/automatic/commit/e91a8d9))

### Documentation

- Removed dead documentation links from the docs index for pages that have migrated to the sibling `automatic-meta` repo: per-agent reference, multi-agent plan, Assets, Sub-Agents, Cloud Library Sync, and the `remote-source-example`. The Assets section (which also pointed at non-existent install-in-automatic badge SVGs) was dropped entirely. Replaced with pointers into `automatic-meta` rather than invented URLs. ([4d270ca](https://github.com/velvet-tiger/automatic/commit/4d270ca), [091a291](https://github.com/velvet-tiger/automatic/commit/091a291))
