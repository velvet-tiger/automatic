...
- **Phase 3b landed.** `library_refresh` gains `download_and_verify` (fetches archive + `.minisig`, verifies with `minisign-verify` against the pinned public key, extracts, hashes every file against the archive's own `manifest.json`) and `apply` (installs verified content into `<root>/library/…`, handles retirements from `retired.json`, updates `settings.library_version`). The Tauri command `apply_library_update` runs the end-to-end flow on demand, and `lib.rs` spawns a background loop that runs the same flow ~30s after boot and every 24h thereafter. Verified end-to-end against `library v0.1.1` on GitHub.
